...
local function root_project(node)
	local root = node
	while root ~= nil do
		if root.kind == symbols.node_kind.PROJECT then
			return root
		end
		root = root.parent
	end
...
M.get_package_data = function(buf, node)
	...
	if node.kind == symbols.node_kind.PROJECT then
		arguments.projectUri = node.uri
	elseif node.kind == symbols.node_kind.CONTAINER then
		arguments.projectUri = root_project(node).uri
		arguments.path = node.path
	elseif node.kind == symbols.node_kind.PACKAGEROOT then
		arguments.projectUri = root_project(node).uri
	...
	elseif node.kind == symbols.node_kind.PACKAGE then
		arguments.projectUri = root_project(node).uri
